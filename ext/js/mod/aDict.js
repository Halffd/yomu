 ...
 */
var hiddenLogs = [];
var lg = (/** @type {HTMLTextAreaElement[]} */ ...v) => {
  if (av('log')) {
    console.log(...v);
  } else {
...
function replace(str, tt, rep = `<s>{}</s>`, reset = true, tgs = ['<sp', 'an>']) {
  try {
    ...
    for (let i = rI; i <= cs.length; i++) {
      ...
      if (cur > 0) {
        let sb = i - pos[0];
        //wn(c, cs[pos[0]], cs[pos[0] + cur], pos[0], cur)
        if (c !== cs[pos[0] + cur]) {
          cur = 0;
        }
      }
...
this.txtImg(false)
  ...
  .catch((error) => {
    ...
    /**
     * @type {{ fullQuery: string; getOptionsContext: () => any; _findDictionaryEntries: (arg0: boolean, arg1: string, arg2: boolean, arg3: any) => any; _displayGenerator: { createTermEntry: (arg0: any) => any; createKanjiEntry: (arg0: any) => any; }; _dictionaryEntryNodes: any[]; _addEntryEventListeners: (arg0: any) => void; _triggerContentUpdateEntry: (arg0: any, arg1: any, arg2: number) => void; _elementOverflowController: { addElements: (arg0: any) => void; }; } | null}
     */
    ...
    if (this.search || this.page) {
      ...
      if (!document.querySelector('.save')) {
        ...
        this.btc('Words-Anki All Cards', async function (/** @type {{ stopPropagation: () => void; }} */ e) {
          e.stopPropagation()
          let si = localStorage.getItem('words') // + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
          si = si.split(' ')
          wn(si)
        ...
        this.btc('-SentenceAudio', async function (/** @type {{ stopPropagation: () => void; }} */ e) {
          e.stopPropagation()
          wn(e)
        ...
        this.btc('Popups', async function (/** @type {{ stopPropagation: () => void; }} */ e) {
          e.stopPropagation()
          wn(e)
        ...
        this.btc('Anms', async function (/** @type {{ stopPropagation: () => void; }} */ e) {
          e.stopPropagation()
          wn(e)
   ...
   * @param {any} i
   */
  async yomi(spl, i, fast = true, fill = false) {
    // debugger;
    this.done = false
    this.stop = false
    //if (!this.card) {
  ...
  /**
   * @param {string | any[]} tt
   * @param {string | any[] | null | undefined} spl
   * @param {string | number} ii
   * @param {number} splL
  ...
  */
  ...
  snav(i, e = null) {
    if (e) {
      e.stopPropagation()
      let p = e.target.closest('.ptxt').nextSibling.firstChild.getAttribute('pos') ?? null;
      this.pos = p ? parseInt(p) : this.pos;
    }
  ...
  sentence(spl = [], ii = 0) {
    ...
    const createNumberSpan = (className, number) => {
      const span = document.createElement('span');
      span.className = className;
    ...
    let spanPrev = createSpan('prev-span');
    let buttonPrev = createButton('navst', 'prev', '\u2039'); // Unicode character for left-pointing arrow

    let spanNext = createSpan('next-span');
    let buttonNext = createButton('navst', 'next', '\u203A'); // Unicode character for right-pointing arrow
    let buttonCopy = createButton('navst', 'copy', 'Copy'); // Unicode character for right-pointing arrow
    let buttonSplit = createButton('navst', 'split', 'CopyAll'); // Unicode character for right-pointing arrow
    let ist = typeof this.spl[0] === 'object' && ii < 1 && this.spl.length > 1 ? `${this.istart}: ` : '';
    let spanNumber = createNumberSpan('number', `${ist}${ii}`); // Assuming `ii` is the identification number


    // Append the elements to their respective parents
    spanPrev.appendChild(buttonPrev);
    spanNext.appendChild(buttonNext);
    spanNext.appendChild(buttonCopy)
    spanNext.appendChild(buttonSplit)
    spanNext.appendChild(spanNumber);
    let ttxt = ctxt.appendChild(document.createElement('p'))
    ctxt.appendChild(spanPrev);
    ttxt.innerHTML = spl[ii];
    ...
    buttonCopy.onclick = function (/** @type {{ stopPropagation: () => void; target: { closest: (arg0: string) => { (): any; new (): any; getAttribute: { (arg0: string): any; new (): any; }; }; }; }} */ e) {
      e.stopPropagation()
      const s = e.target.closest('.ptxt').getAttribute('s')
      if (av('warn')) console.warn(s)
      this._copyText(s)
    }.bind(this)
    buttonSplit.onclick = function (/** @type {{ stopPropagation: () => void; target: { closest: (arg0: string) => { (): any; new (): any; getAttribute: { (arg0: string): string; new (): any; }; }; }; }} */ e) {
      e.stopPropagation()
      let i = parseInt(e.target.closest('.ptxt').getAttribute('ii'))
      let s = ''
      if (typeof this.spl[0] === 'object' && this.spl.length > 1) {
        s = this.spl[this.istart].slice(i).join('\n');
        for (let i = this.istart + 1; i < this.spl.length; i++) {
          const t = this.spl[i].join('\n');
          s += '\n' + t
        }
      } else {
        s = this.spl[0].slice(i).join('\n');
      }
      if (av('warn')) console.warn(s)
      this._copyText(s)
    }.bind(this)
    ...
    try {
      if (this.start && ii >= 0) {
        ...
        if (isKana) {
          ...
          if (this.var('tl') && !this.saving) await this.translate(spl, ii, ptxt)
        } else {
  ...
  setup(ptxt = null) {
    ...
    if (this.saving) {
      this.modK.className = 'modK saving'
      if (this._i) {
  ...
  async loadMore(istart, st = null) {
    ...
    if (istart < 0 || istart >= sgroup.length) {
      istart = istart < 0 ? 0 : sgroup.length - 1
      this.istart = istart
    }
    try {
 ...
 */
  ...
  async update(q, from = null) {
    ...
    if (this.var('run')) {
      ...
      if (fq) {
        ...
        function recursiveArrayLength(arr) {
          let count = 0;
        ...
        const ln = recursiveArrayLength(this.spl)  //.length;
  ...
  /**
   * @param {{ (_: any): boolean; (_: any): boolean; (_: any): boolean; (_: any): boolean; (): any; }} conditionFunction
   */
  ...
  */
  ...
  async wait() {
    await this.waitFor((/** @type {any} */ _) => document !== null)
    await this.waitFor((/** @type {any} */ _) => this.w() == true)
    if (av('log')) console.log('the wait is over!')
  }

  async autoClick() {
    await this.waitFor((/** @type {any} */ _) => document !== null)
    await this.waitFor((/** @type {any} */ _) => this.ac() == true)
  ...
  moveElem(shift, pos = 0, elem = null, dir = null) {
    ...
    try {
      ...
      if (parent) {
        ...
        if (shift) {
          if (pos > 0 || (pos == 0 && this.pos > this.ppos)) {
            parent.insertBefore(d, a); // a-d left, d-a right
          } else {
            parent.insertBefore(a, d); // a-d left, d-a right
          }
          d.setAttribute('pos', this.ppos)
          a.setAttribute('pos', this.pos)
          this.cur(a);
        } else {
          if (pos > 0 || (pos == 0 && this.pos > this.ppos)) {
            parent.lastChild.setAttribute('pos', this.pos)
            a.setAttribute('pos', parent.lastChild.getAttribute('pos'))
            parent.appendChild(a); // end
          } else {
            parent.firstChild.setAttribute('pos', this.pos)
            a.setAttribute('pos', parent.firstChild.getAttribute('pos'))
 ...
 * @param {number} d - The direction of movement (-1 for backwards, 1 for forwards).
 * @param {number} pos - The starting position.
 * @param {NodeListOf<Element>} _t - Unused parameter.
 * @param {string | any[] | NodeListOf<Element>} b - The array-like structure.
 * @returns {number} - The new position after movement.
 */
  mv = (d, pos, _t, b) => {
    let l = pos;
    l += 1 * d;
    ...
    try {
      ...
      if (b[pos] && b[pos].parentNode) {
        ...
        throw new Error('Invalid input: b[pos] does not exist or has no parentNode');
      }
    } catch (error) {
      console.error('An error occurred:', error);
      return -1;
    ...
    if (d == -1) {
      try {
        if (av && av('warn')) {
          if (b[l] && b[l].parentElement) {
            console.warn(pos, l, b[l].parentElement.children.length, b[l].parentElement.children[0]);
          } else {
            throw new Error('Invalid input: b[l] does not exist or has no parentElement');
          }
        }
      } catch (error) {
        console.error('An error occurred:', error);
      ...
      while (m < b.length) {
        m += 1;
        lm = l - m;
        if (av && av('warn')) console.warn(`Iteration: ${m}, lm: ${lm}, l: ${l}, b[lm]: ${b[lm]}`); // Log iteration details
        try {
          if (b[l] && b[lm] && b[l]?.parentNode?.contains(b[lm])) {
  ...
  btn(_wt) {
    ...
    function createSettingsInputTemplate(id, label, description, border) {
      ...
      const ev = (/** @type {{ stopPropagation: () => void; target: any; }} */ e) => {
        e.stopPropagation()
        const input = e.target
        if (av('warn')) console.warn(e, input)
        const iid = input.id
...
<!--                    <div class="settings-item-description">${description}</div>-->
    ...
    function setupToggleSwitch(container, localStorageKey, _fallbackValue) {
      ...
      toggleCheckbox.addEventListener('change', function (/** @type {{ stopPropagation: () => void; }} */ eee) {
        eee.stopPropagation()
        updateToggleState()
      }, false)
    ...
    const toggleButton = (/** @type {HTMLInputElement | null} */ button, /** @type {string} */ key) => {
      ...
      button.onclick = (/** @type {{ stopPropagation: () => void; }} */ e) => {
        e.stopPropagation()
  ...
  tr(text, search, replace) {
    ...
    const t = text.replace(regex, function (/** @type {any} */ chr) {
...
*/
  ...
  /**
   * @param {any[]} z
   * @param {string | number} ik
   * @param {HTMLParagraphElement} h
   * @param {any} optionsContext
   * @param {{ innerHTML: string; querySelectorAll: (arg0: string) => { innerHTML: any; }[]; }} pw
   */
  ...
  /**
   * @param {{ getAttribute: (arg0: string) => number; parentElement: { getAttribute: (arg0: string) => any; }; } | null} b
   * @param {number | undefined} istart
   */
  ...
  /**
   * @param {string | any[]} z
   * @param {{ innerHTML: string; querySelectorAll: (arg0: string) => any; }} h
   * @param {any} _optionsContext
   */
   ...
   * @param {any} r
   * @param {any} m
   * @param {{ appendChild: (arg0: HTMLDivElement) => void; querySelector: (arg0: string) => any; }} c
   * @param {number} o
   */
  def(t, r, m, c, o) {
    if (o == 0) {
  ...
  async frequency(word, dbs = this.jpdb) {
    ...
    const entry = await db.find((/** @type {{ reading: any; }[]} */ entry) => entry[0] === word || entry[2].reading === word) ?? -1
  ...
  async reading(word, dbs = this.jpdb) {
    let db = await this.is(dbs)
    const entry = db.find((/** @type {any[]} */ entry) => entry[0] === word)
  ...
  /**
   * @param {string | any[]} dictionaryEntries
   */
        ...
        try {
          ...
          for (const y of yc) {
            ...
            if (this.var('fq')) {
              ...
              fq = fqs.reduce((/** @type {any} */ partialSum, /** @type {any} */ a) => partialSum + a, 0) / fqs.length
