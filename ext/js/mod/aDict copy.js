/* global aDict, Note, isIndexInsideElement, aDeck, aTag, aAll, aModel, api,
getWords, merge, chrome, unconjugate */
class Dict {
    ...
    var ankisAll = async (op = 0) => {
      ...
      await Promise.all([def, anm, jp2, jpmn, pop])
        .then((results) => {
          ...
          note.svDiv(document.querySelector('.save'), un, undefined, true)
...
this.txtImg(false)
  ...
  .catch((error) => {
      ...
      const makeElem = async (tt, spl, ii, splL, I, w, line, emph, ltp, tts, mns = null, y = null, elem = null, o = this.var) => {
        ...
        try {
          ...
          if (ltpChildren.length === 0) {
            ...
            console.warn(I, searchString, pattern2, firstChildText)
          ...
          /* const replacedText = firstChildText.replace(pattern, `<span>${replacement}</span>`)/*pattern, (match, _capturedGroup1, offset, input) => {
            console.warn(match)
                          let char
                          let r = true
                          if (r) {
        ...
        if (elem.querySelector('.gloss-definitions').innerHTML?.length < 1 || (f != -1 && f < this.fval && this.var('jd')) || (!isKana && o('kan'))) {
        ...
        if (this.fl) {
          if ((o('auto') && !o('yc') && !o('both')) || (il < 5 && splL < 3)) {
            sv(elem, 1, tt, spl[ii])
          }
        }

        if (this.var('kanji')) { //(!this.slow || o('fq') || (o('yc') && !o('both'))) {
    ...
    <link rel="stylesheet" type="text/css" href="./moe.css">`
            ...
            if (this.modK.querySelector('.vis')) {
      ...
      const runner = async (sp, t, done, modP = this.modP) => {
        ...
        for (const ii in c) {
          ...
          try {
            if (this.lang == 'id' && I > 2) {
              let tt = display.fullQuery.toLocaleLowerCase()
              const tls = this.modT.appendChild(document.createElement('div'))
              tls.id = 'mtl'
              const tll = tls.appendChild(document.createElement('div'))
              const mk = tll.appendChild(document.createElement('span'))
      ...
      const loadMore = async (istart, st = null) => {
        ...
        pos = 0
        await yomi(ss, istart, true) //.bind(this) // o('slw'))
        if (pos == 0 && st != null && !o('yc')) {
          const b = document.querySelectorAll('.vis')
      ...
      const update = async (q) => {
        ...
        if (run) {
          ...
          if (o('slw')) {
            ...
            if (o('hk') || this.lis) {
              istart = 0
              await yomi.bind(this, sgroup[istart], istart)()
            } else {
              await yomi.bind(this, spl[istart + 1], istart + 1)()
            }
          } else {
            await yomi.bind(this, spl, 0, false)()
          }
          this.start = true
        }
        this.new = true
