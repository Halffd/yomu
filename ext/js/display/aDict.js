...
//const dic = new Dict()
class Dict {
    constructor(display, audio, japaneseUtil) {
            ...
            for (let i in zs) {
                ...
                if (zs[i]) {
                    ...
                    if (this.search) {
                        ...
                        lng = document.body.appendChild(document.createElement("div"))
                    }
                    let ue = (str) => {
        ...
        if (this.search || this.page) {
                                            ...
                                            elem.addEventListener("click", function (ele) {
                                                ...
                                                try {
                                                    ...
                                                    if (kj) {
                                                        ...
                                                        en = en.innerText
                                                        //en = cpn.innerText
                                                        sv(en, xt)//localStorage.setItem('save', `${save} ${en}`)
                                                    } else {
                                                        sv(k.innerHTML, xt) //localStorage.setItem('save', `${save} ${k.innerHTML}`)
                                                    }
                                                } catch (err) {
                                                    console.error(err);
                                                }
            ...
            let slow = async (spl, i) => {
                ...
                try {
                    ...
                    if (mode == 50 && window.navigator.onLine) {
                        for (ii in spl) {
                            if (this.prevent(80)) {
                        ...
                        if (o('rd') && (!o('kan') || !o('fq'))) {
                            for (let d of this.modP.children) {
                                if (d.children) {
                        ...
                        if (o('rd') || o('kan') || o('fq')) {
                            for (let v of cc) {
                                try {
                                    try {
                                        let vt = v.querySelector('.query-parser-segment-text').innerText
                        ...
                        for (let elem of document.querySelectorAll('.vis')) {
                            ...
                            if (!elem.querySelector('.gloss-content') && (this.var('yc') || this.wiki || gd <= 1 || iji >= 0)) {
                                try {
                        ...
                        for (ii in c) {
                            try {
                                if (ii >= 0) {
                                    ...
                                    let tp = ''
                                    lil = tp.length
                                    console.log(`${parseInt(ii) + 1}/${c.length}.  : `, ii, tt, tts, ix, ind)
                                    let cnt = lil
                                    if (c[ii].classList[1] == 'newline') {
                                        ix += 1
                                        this.nl = true
                                        this.nll = true
                                        console.log(`===-=>${this.nl}`)
                                    ...
                                    if (isKana) {
                                        ...
                                        //const makeNextPromise = (t) => async () 
                                        //{
                                        let f = await this.dict(k, tt, optionsContext, ii, tts, spl[ix], 0)
                                        console.log(f)
                                        //return f
                                        //}
                                        //promiseChain = promiseChain.then(makeNextPromise(''))
                    ...
                    if (!document.querySelector('.save')) {
                        ...
                        this.t.innerHTML += `<h5 class="save">${sav}</h5>`
                    ...
                    await this.tl(tlx, this.lang, 'en').then((txt) => {
                        console.log('tl----------------------')
                        console.dir(txt)
                        let tl = txt
                        for (let i in tl) {
                            ...
                            if (tl[i] !== null && tl[i].length > 0 && typeof tl[i] == 'object') {
                                for (let j in tl[i]) {
                                    if (tl[i][j] !== null && tl[i][j].length > 0 && typeof tl[i][j] == 'object') {
                                        document.querySelector('.save').insertAdjacentHTML('beforebegin', `<p>${tl[i][j][0]}</p>`)
                                        //[k]
                                        //this.t.innerHTML += '</br>'
                                    }
                                }
            ...
            document.addEventListener('keydown', e => {
                ...
                if (ret) { return }
                let b = document.querySelectorAll('.nav')
                let ps = []
                ...
                if (ki <= 9) {
                    ...
                    console.log(`${dir}`)
                    try {
                        let t = b[pos].querySelector('.query-parser-segment-text').innerText
                        let x = -1
                        console.warn(t, posr);
                        for (let y of ys) {
                            let z = y.querySelector('.kj').innerText
                            //console.warn(z);
                    ...
                    posr = [pos, b[pos].style.border, ps]
            ...
            setInterval(async () => {
                ...
                if (run) {
                    ...
                    let fq
                    if (this.search) {
                        fq = this._display.fullQuery
                    } else {
                        fq = this.v
                    }
                    if (!this.prevent(90) && nv && ((fq != this.lq) || (this.frst && this.w3() == false))) {
                        try {
                        ...
                        if (mode >= 50 && o('slw')) {
                            ...
                            if (this.read) {
                                ...
                                console.warn(ps);
                                await slow(spl[istart + 1], istart + 1)
                            }
                        } else {
                            //await runner()
                        }
    ...
    async autoClick() {
        ...
        if (this.ac()) {
            this._addAnkiNote(0, "term-kanji")
        }
    }
    ...
    async dictionary(k, optionsContext, i, t, r, ln, nl, elem) {
        console.dir(elem)
        optionsContext = elem.querySelector('.kj').innerText.replace(/[&\/\\#,+()$~%.'":*?<>{}【】]/g, '')
        console.log(optionsContext);
        /*if (k == 1000) {
            var xclx = function (ele) {
                ele.stopPropagation()
                console.dir(ele, ele.target)
                let save = localStorage.getItem('save')
                try {
                    let kj = ele.target.querySelector("span.kj")
                    //console.dir(kj)

                    sv(kj.innerHTML) //localStorage.setItem('save', `${this.save} ${kj.innerHTML}`)
                } catch (err) {
                    console.error(err);
                }
                if (elem.style.width != '') {
            ...
            elem.onclick = xclx
            elem.children[1].children[0].onclick = xclx
            //console.warn(elem,elem.children[1]);
            return true
        }*/
        let num = false
                ...
                if (this.var('fq') || this.var('yc')) {
                    ...
                    try {
                        for (let y of yc) {
                            ...
                            //if()
                            /*let cpn = document.createElement('div')
                            let en = y.querySelector('.headword-text-container').cloneNode(true)
                            let ek = y.querySelector('')
                            //en = y.cloneNode()
                            console.warn(en.innerHTML);
                            for (let r of en.querySelectorAll('a')) {
                                r.remove()
                            }
                            en.querySelector('.headword-reading').remove()
                            //en.querySelector('rt').remove()
                            ...
                            if (this.var('yc')) {//en.indexOf(elem.querySelector('.kj').innerText) != -2) {llz
                                ...
                                y.querySelector('.entry-header').appendChild(elem.querySelector('.part'))
                                y.querySelector('.entry-header').style.border = '2px solid purple'
                                y.querySelector('.entry-header').className += ' title'
                                y.querySelector('.headword-term').className += ' kj'
                                let o = this.var
                                if (!(o('rd') || O('fq') || O('kan'))) {
                                    y.querySelector('.headword-term').className += ' nav'
                                }
                                if (this.var('yc')) {
                                    elem.querySelector('.tit').remove()
                                    elem.prepend(y)
                                }
                                //  //yc = yc.slice(0, 1)
                                //       break
                    ...
                    if (iji && y.length >= 1) {
                        for (let i in y) {
                            ...
                            if (ijii) {
                                ...
                                console.dir(result)
                                try {
                                    elem.querySelector('dd').innerHTML += `<li>[${y[i]}] Onyomi: ${result[kx].kunyomi.join(' ')}</br>Kunyomi: ${result[kx].onyomi.join(' ')}</br>${result[kx].definitions.join(', ')}</li>`
                                } catch (error) {
                                    console.log(error)
                                }
  ...
  a.href = imageBase64*/
    ...
    saveAdd(t, txt) {
        ...
        localStorage.setItem('day', date.toString());
        let l = JSON.parse(save);
        l.push([t, txt, Date.now(), d, id]);
        let w = [];
        let is = [];
        for (let i of l) {
          w.push(i[0]);
        ...
        l = Array.from(seen); // or l = [...seen];
        console.warn(l, w, uniqueArr);
        let wds = JSON.parse(word)
        wds.push(...w)
        let art = wds
        let uniqueArray = art.filter(function (item, pos) {
            return art.indexOf(item) == pos;
        ...
        wds = art
        let ws = JSON.stringify(wds);
        let str = JSON.stringify(uniqueArr);
        localStorage.setItem('save', str);
        localStorage.setItem('words', ws);
      }
}