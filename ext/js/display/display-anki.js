...
=======
        this._dictionaryEntryTypeModeMap = new Map([['kanji', ['kanji']], ['term', ['term-kanji', 'term-kana']]]);
>>>>>>> origin/master
...
=======
        ...
        let ff = async () => {
            for (let i in zs) {
                ...
                if (zs[i]) {
                    ...
                    let lng = this._display._container.appendChild(document.createElement("div"))
                    let ue = (str) => {
            ...
            let lis
            if (l >= 0) {
                lis = this._display._container.querySelectorAll('li')
            }
            let tll
            for (let ijk in lis) {
                if (ijk >= 0) {
                    tll = await this.tl(lis[ijk].innerHTML, 'id', 'en')
                    //console.dir(lis[i])
                    console.dir(tll)
                    for (let i in tll) {
        ...
        var lng = this.lang
        if (this._display._pageType == "search") {
            if (this.wiki && l >= 0) {
                ff()
            }
            let bi = document.createElement('div')
            bi.innerHTML += `<button id="btn">Toggle</button><input id='sl'/><button id="wk">Wiki</button><button id="exe">O</button>`
            ...
            ob.onclick = function () {
                ...
                if (run) {
                    //mode = 0
                    ob.setAttribute('aria-pressed', '1')
                    run = false
                } else {
                    ob.setAttribute('aria-pressed', '0')
                    run = true
                    //console.log(at, run, mq.parentElement)
                }
        ...
        this.lq = ''
        if (this._display._pageType == "search") {
            this.b = this._display._container.appendChild(document.createElement("div"))
            let bs = this.btn(this)
            for (let i in bs) {
                document.body.prepend(bs[i])
            }
            const TinySegmenter = function () {
                var patterns = {
                    "[一二三四五六七八九十百千万億兆]": "M",
                    "[一-龠々〆ヵヶ]": "H",
                    "[ぁ-ん]": "I",
            ...
            let runner = async () => {
                ...
                try {
                    ...
                    this.t.id = 'tl'
                    this.modP = this._display._container.appendChild(document.createElement("div"))
                    //console.dir(this.modK)
                    //this.modK.innerHTML = '99999999999999'
                    this.mod = this._display._container.appendChild(document.createElement("div"))
                    this.mod.id = 'mod'
                    this.modC = this._display._container.appendChild(document.createElement("footer"))
                    this.modC.id = 'def'
                    //this.modK.prntElement.style.display = 'flex'
                    this.limit = '13em'
                    this.wid = '13em'
                    //document.querySelector('.content-body-inner').style.removeProperty('width')
                    this.modP.style.display = 'inline-block'
                    this.mod.style.display = 'flex'
                    this.mod.style.flexWrap = 'wrap'
                    this.mod.style.flex = '5%'
                    let ii
                        ...
                        try {
                            if (ii >= 0) {
                                ...
                                if (isKana) {
                                    ...
                                    for (let i in ws) {
                                        let r1
                                        let iss = japaneseUtil.isStringPartiallyJapanese(ws[i])
                                        if (iss) {
                                            if (japaneseUtil.isStringEntirelyKana(ws[i])) {
                                                let r0 = japaneseUtil.convertKatakanaToHiragana(ws[i])
                                                r1 = japaneseUtil.convertToRomaji(r0)
                                                ptxt.querySelector('rt').innerHTML += r1
                                            }
                                            let verb = []
                                            //while(verb.length == 0)
                                            verb = unconjugate(ws[i])
                                            if (ws.length - i + 1 + 2 >= 0) {
                                                verb = unconjugate(ws[i] + ws[parseInt(i) + 1] + ws[parseInt(i) + 2])
                                            }
                                            let f = await this.dict(k, ws[i], optionsContext, ii, i, ws, r1)
                                            console.warn(k, ws[i], ii, i, ws.length, r1, iss, verb, verb.length, ws[parseInt(i) + 1])
                                        }
                                    }
                                    //console.log(f)
                                    //return f
            ...
            setInterval(async () => {
                ...
                //    console.log(prog.getAttribute('data-active'), this.lq, this.w2(), this.w3())
                    if ((this.frst && this._display.fullQuery != this.lq) || (this.frst && (this.w2() == true || this.w3() == false))) {
                            await runner()
                            ex.onclick = runner
                            this.frst = false
...
=======
        this._display.hotkeyHandler.registerActions([['addNoteKanji', () => {
            this._tryAddAnkiNoteForSelectedEntry('kanji');
        }
        ], ['addNoteTermKanji', () => {
            this._tryAddAnkiNoteForSelectedEntry('term-kanji');
        }
        ], ['addNoteTermKana', () => {
            this._tryAddAnkiNoteForSelectedEntry('term-kana');
        }
        ], ['viewNote', this._viewNoteForSelectedEntry.bind(this)]]);
>>>>>>> origin/master
...
=======
        ...
        try {
            ...
            for (let ii in results) {
                for (let i in results[ii].definitions) {
                    ...
                    //if (l > this.limit) {
                    //break}
                    let o = results[0].definitions[i].entries.join(',')
                                ...
                                if (o.type === undefined) {
                                    ...
                                    for (let i in o) {
                                        ...
                                        for (let j in o[i].content) {
                                            if (o[i].content[j].content !== undefined) {
            ...
            ct.ondblclick = () => {
                if (ct.querySelector('.entry-body').style.width != '') {
                    ct.querySelector('.entry-body').style.width = ''
                    ct.querySelector('.entry-body').style.height = ''
                    ct.style.flex = ''
                } else {
                    ct.querySelector('.entry-body').style.height = this.limit;
                    ct.querySelector('.entry-body').style.width = this.wid;
                    ct.style.flex = '5%'
                }
            }
            //ct2.style.flex = '5%'
    ...
    tl(w, from, to) {
        let url = encodeURI(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=${from}&tl=${to}&dt=t&q=${w}`)
    ...
    async wkt(w, l) {
        let url = `https://en.wiktionary.org/w/api.php?titles=${w}&action=query&prop=extracts&format=json`
        if (l == 0) {
            url = `https://zh.wiktionary.org/w/api.php?titles=${w}&action=query&prop=extracts&format=json`
        } if (l == 1) {
            url = `https://ja.wiktionary.org/w/api.php?titles=${w}&action=query&prop=extracts&format=json`
        } if (l == 2) {
            url = `https://id.wiktionary.org/w/api.php?titles=${w}&action=query&prop=extracts&format=json`
        }
        //return new Promise(function (resolve, reject) {
        ...
        xhr.responseType = 'json'
        xhr.open('GET', url);
        xhr.setRequestHeader("Content-type"," text/html;charset=UTF-16");
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 700) {
                resolve(xhr.response);
            } else {
    ...
    async originate(z, h, optionsContext) {
        ...
        lu = lu[lu.length - 1]
        let llang = lu.querySelectorAll('h2')
        let oorgn = lu.querySelectorAll('p')
        let jjp = lu.querySelectorAll('ul')
        let lc = lu.children
        let lt = false
        let ad = false
        let or = false
        ...
        for (let m in lc) {
            try {
                //console.log(optionsContext, lc[m])
                if (lc[m].tagName == 'H2') {
        ...
        let o = ''
        let kj = await this.web(`https://www.kanjipedia.jp/search?kt=1&sk=leftHand&k=${optionsContext}`, 0)
        //console.dir(kj)
        h.innerHTML += '</br><div class="kjpSearch">' + ue(kj) + '</div>'
        let ku = h.querySelectorAll('.kjpSearch')
        ku = ku[ku.length - 1]
        try {
            let kurl = ku.querySelector('#resultKanjiList > li > a').getAttribute('href')
            kj = await this.web(`https://www.kanjipedia.jp${kurl}`, 0)
            h.innerHTML += '</br><div class="kjp">' + ue(kj) + '</div>'
            ku = h.querySelectorAll('.kjp')
            ku = ku[ku.length - 1]
            let os = []
            os.push('</br>Origin: ' + ku.querySelector('#kanjiRightSection > ul > li.naritachi > div:nth-child(2) > p').innerHTML)
...
=======
        const { general: { resultOutputMode, glossaryLayoutMode, compactTags }, anki: { tags, duplicateScope, duplicateScopeCheckAllModels, suspendNewCards, checkForDuplicates, displayTags, kanji, terms, noteGuiMode, screenshot: { format, quality }, downloadTimeout }, scanning: { length: scanLength } } = options;
>>>>>>> origin/master
...
=======
        if (this._updateDictionaryEntryDetailsToken !== token) {
            return;
        }
>>>>>>> origin/master
...
=======
        if (!(dictionaryEntryDetails !== null && dictionaryEntryIndex >= 0 && dictionaryEntryIndex < dictionaryEntries.length && dictionaryEntryIndex < dictionaryEntryDetails.length)) {
>>>>>>> origin/master
            return;
        }
...
>>>>>>> origin/master
        ...
        try {
<<<<<<< HEAD
            const { note, errors, requirements: outputRequirements } = await this._createNote(dictionaryEntry, mode, requirements, true);
...
=======
        ...
        content.style.backgroundColor = 'rgba(80,80,80,20)'
        //console.dir(node)
        content.onclick = () => {
            console.log({
                ankiNoteErrors: errors
            });
...
=======
            results[index].modeMap.set(mode, {
                mode,
                note,
                errors,
                requirements,
        ...
        //console.log(notes.join(', '), noteTargets, results);
        setTimeout(() => {
            try {
                let entry = this._getEntry(0);
                let button = entry.querySelector(`button[data-mode="term-kana"]`)
                //[data-mode="${mode}"]`) 
                //const button = this._adderButtonFind(dictionaryEntryIndex, mode);
                if (button !== null && !button.disabled) {
...
<<<<<<< HEAD
    async _createNote(dictionaryEntry, mode, requirements, anki) {
...
=======
    async _createNote(dictionaryEntry, mode, requirements) {
        const context = this._noteContext;
        //console.log(mode)
        //if (mode == 'term-kana'){            mode = 'term-note'        }
        let modeOptions = this._modeOptions.get(mode);
        if (typeof modeOptions === 'undefined') {
            throw new Error(`Unsupported note type: ${mode}`);
        }
        const template = this._ankiFieldTemplates;
        const { deck: deckName, model: modelName } = modeOptions;
        const fields = Object.entries(modeOptions.fields);
...
=======
        ...
        try {
            ...
            const displayErrors = (e.message === 'Mode not supported' ? [this._display.displayGenerator.instantiateTemplateFragment('footer-notification-anki-view-note-error')] : void 0);
>>>>>>> origin/master
