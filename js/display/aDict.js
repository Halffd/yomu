...
class Note {
    ...
    async saveAdd(t, txt, def = '', fq = [], tags = [], html = '', moe = false, audio = [], image = [], clip = '', yc = false, read = '') {
        let sz = ''
        try {
            const log = {
                dict: this.dic,
        ...
        try {
            ...
            const isStringInSet = bs.includes(t);
            if(isStringInSet){
                return isStringInSet;
            }
            bs.push(t);
            try {
                localStorage.setItem('wordbk', wrd + ' ');
                localStorage.setItem('wordsetbk', bks);
            } catch (error) {
                console.error("Error updating localStorage:", error);
            }
            ...
            note.svDiv(saveDiv, sz, flag)
        } catch (e) {
            console.error(e);
        }
    }
    svDiv(saveDiv, sz = null, flag = '') {
        if (!sz) {
    ...
    svClk(elem, cx = 0, tw = '', tx = '', yc = null, clip = '', img = [], snd = []) {
        let fq = []
        let df = ''
        ...
        if (cx == 0) {
            ...
            if (k) {
                ...
                let rd = elem.querySelector('rt')
                //en = cpn.innerText
                //
                //async saveAdd(t, txt, def = '', fq = [], tags = [], html = '', moe = false, audio = [], image = [], clip = '', yc = null) {
                note.saveAdd(wn, xt, df, fq, tag, ht, false, snd, img, clip, true, rd)//localStorage.setItem('save', `${save} ${en}`)
            } else {
                try {
                    df += elem.querySelector('dt').innerText
                } catch { }
                note.saveAdd(k.innerText, xt, df, undefined, tag, ht, true, snd, img, clip, true) //localStorage.setItem('save', `${save} ${k.innerHTML}`)
            }
        } else {
            try {
                df += elem.querySelector('dt').innerText
            } catch { }
            note.saveAdd(tw, tx, df, undefined, tag, ht, true, snd, img, clip)
        }
    }
...
class Dict {
    constructor(display = null, audio = null, japaneseUtil = null, anki = null) {
...
this.txtImg(false)
  ...
  .catch((error) => {
    console.error('Error retrieving text:', error);
        ...
        this.tb = true
        this.tbs = 0
        this.wiki = this.wiki = localStorage.getItem('wkt') === 'true'; //localStorage.getItem('wkt') === 'true' ? true : false;
                                ...
                                try {
                                    ...
                                    if (ana && mns[ii].innerHTML) {
                                        ...
                                        try {
                                            ...
                                            tit.innerHTML = `<div class="tit"><span class="kj">${tt}</span><span class="rd">${tts}</span></div>`
                                        } catch (clrr) {
                                            console.error(clrr);
                                        }
...
console.warn(currentIndex);
                                        ...
                                        if ((isPart && ii > 0) || (tt.length < this.min)) {
                                            let ttt = [tt, 8]
                                            line.push(ttt)
            ...
            let bnv = document.querySelector('content')
            for (let i in bs) {
                document.body.prepend(bs[i])
            }
            ...
            var wb = document.getElementById('wk')
            var sl = document.getElementById('sl')
            var ki = document.getElementById('kan')
            var pc = document.getElementById('pt')
            var wt = document.getElementById('wt')
            var ht = document.getElementById('ht')
            var mn = document.getElementById('min')
            var as = document.getElementById('aut')
            mn.value = parseInt(localStorage.getItem('freq'))
            sl.value = localStorage.getItem('lng') + '.Lang'
            wt.style.backgroundColor = 'red'
            ht.style.backgroundColor = 'blue'
            var wtv = this.wid
            var htv = this.limit
            ht.value = htv + '|'
            wt.value = wtv + '_'
            var vs = []
            var stt = []
            var button = function (b, k) {
                let vl = localStorage.getItem(k) == 'true' ? true : false
                console.warn(b, k, vl);
                stt.push(vl)
                if (vl == true) {
                    //wv = false
                ...
                b.onclick = function (e) {
                    e.stopPropagation()
                    let n = e.target.id
                    let lg = localStorage.getItem(n) == 'true' ? false : true
                    let v = lg
            ...
            var vbs = []
            for (let v in vs) {
                vbs.push(document.getElementById(vs[v]))
                button(vbs[v], vs[v])
            }
            console.warn(vs, vbs, stt);
            if (pt == true) {
                //wv = false
            ...
            ki.onclick = function () {
                let lg = kan == true ? false : true
                v = lg
                localStorage.setItem('kan', v);
                if (lg == null) {
                    v = true; //id-ID
                    localStorage.setItem('kan', true);
                }
                kan = v
                if (kan == true) {
                    //wv = false
            ...
            as.onclick = function () {
                let lg = aut == true ? false : true
                v = lg
                localStorage.setItem('auto', v);
                if (lg == null) {
                    v = true; //id-ID
            ...
            wt.onchange = () => {
                let lg = wt.value
                v = lg
                localStorage.setItem('wt', v);
                if (lg == null) {
                    v = '13em'; //id-ID
                    localStorage.setItem('wt', '13em');
                }
                wtv = v
                console.dir(v)
            }
            ht.onchange = () => {
                let lg = ht.value
                v = lg
                localStorage.setItem('ht', v);
                if (lg == null) {
                    v = '13em'; //id-ID
                    localStorage.setItem('ht', '13em');
                }
                htv = v
                console.dir(v)
            }
            mn.onchange = () => {
                ...
                if (lg == null) {
                    v = '10000'; //id-ID
                    localStorage.setItem('freq ', '10000');
                }
                //min = v
                console.dir(v)
            }
            ...
            ob.setAttribute('aria-pressed', '0')
            var ft = this.frst
            let wvv = localStorage.getItem('wkt') === 'true' ? true : false
            if (wvv == true) {
                //wv = false
            ...
            wb.onclick = function () {
                let wv = localStorage.getItem('wkt') == 'true' ? true : false
                if (wv == true) {
                    wv = false
                    wb.style.backgroundColor = 'red'
                } else {
                ...
                console.log(wv)
                localStorage.setItem('wkt', wv)
            }
            ob.onclick = function () {
                //let at = ob.getAttribute('aria-pressed')
                //let mq = document.querySelector('#m')
                let r = localStorage.getItem('run') == 'true' ? false : true
                run = !run
                localStorage.setItem('run', r)
            }
            sl.onchange = () => {
                let lg = sl.value
                v = lg
            ...
            if (true || run) {
                ...
                if (!document.querySelector('.save')) {
                    ...
                    saveDiv.onclick = function (e) {
                        ...
                        let si = /*localStorage.getItem('word') + ` ${localStorage.getItem('wordbk')}`*/ document.querySelector('.save .w').innerText
                        cpy(si)
                    }
        ...
        for (var i = 0; i < 7; i++) {
            ...
            bt.onclick = function (i) {
                ...
                console.log(i); if (i === 0) {
                    ...
                    let x = document.querySelector('.scan-disable');
                    console.warn(x);
                    x.style.display = "block";
                    if (x.style.display != "none") {
                        //x.style.display = "none";
