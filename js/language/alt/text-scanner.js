...
/*
        ...
        //document.addEventListener('keydown', this._handleArrowKeyNavigation.bind(this));
        const getCurrentX = (event) => {
...
document.addEventListener('keydown', (event) => {
    const key = event.key;
    if (key === 'ArrowLeft') {
        // Process text to the left
        const x = getCurrentX(event);
        const y = getCurrentY(event);
        this._searchAt(x - 1, y, inputInfo); // Adjust the X coordinate to move left
    } else if (key === 'ArrowRight') {
        // Process text to the right
        const x = getCurrentX(event);
        const y = getCurrentY(event);
        this._searchAt(x + 1, y, inputInfo); // Adjust the X coordinate to move right
    } else if (key === 'ArrowUp') {
        // Process text above
        const x = getCurrentX(event);
        const y = getCurrentY(event);
        this._searchAt(x, y - 1, inputInfo); // Adjust the Y coordinate to move up
    } else if (key === 'ArrowDown') {
        // Process text below
        const x = getCurrentX(event);
        const y = getCurrentY(event);
        this._searchAt(x, y + 1, inputInfo); // Adjust the Y coordinate to move down
    }
});
    }*/

    /*_handleArrowKeyNavigation = function (event) {
        if (!this._enabled) {
          return;
        }
        ...
        if (arrowKeys.includes(event.key)) {
          ...
          if (selection.rangeCount === 0) {
            ...
            const currentTextSource = DocumentUtil.findContainingTextSource(range.startContainer);
            if (!currentTextSource) {
                //return;
            }
            console.warn(event,selection,currentTextSource);
      
          const direction = this._getDirectionFromArrowKey(event.key);
          const nextTextSource = this._getNextTextSource(currentTextSource, direction);
          if (!nextTextSource) {
            //return;
          }
      
          // Update the current text source and perform text selection
          this.setCurrentTextSource(nextTextSource, range.startContainer, direction);
        }
      };
      
      _getDirectionFromArrowKey = function (key) {
        switch (key) {
          case 'ArrowLeft':
            return 'left';
          case 'ArrowRight':
      ...
      _getNextTextSource = function (currentTextSource, direction) {
        let nextTextSource = null;
        let currentNode = currentTextSource.node;
        console.warn(currentNode);
      
        // Traverse the DOM based on the specified direction
        switch (direction) {
          case 'left':
            // Logic for selecting text to the left of the current text source
            nextTextSource = this._getPreviousTextSource(currentNode);
            break;
          case 'right':
            // Logic for selecting text to the right of the current text source
            nextTextSource = this._getNextTextSource(currentNode);
            break;
          case 'up':
            // Logic for selecting text above the current text source
            nextTextSource = this._getAboveTextSource(currentNode);
            break;
          case 'down':
            // Logic for selecting text below the current text source
            nextTextSource = this._getBelowTextSource(currentNode);
            break;
          default:
            break;
        }
      ...
      _getPreviousTextSource = function (currentNode) {
        // Find the previous text source by traversing the DOM
        let previousNode = currentNode.previousElementSibling;
        while (previousNode) {
          const previousTextSource = DocumentUtil.findContainingTextSource(previousNode);
          if (previousTextSource) {
            return previousTextSource;
          }
          previousNode = previousNode.previousElementSibling;
        }
      
        // If no valid previous text source is found, traverse up the DOM tree
        const parentNode = currentNode.parentNode;
        if (parentNode && parentNode !== document.body) {
          return this._getPreviousTextSource(parentNode);
        }
      
        return null;
      };
      
      _getNextTextSource = function (currentNode) {
        // Find the next text source by traversing the DOM
        let nextNode = currentNode.nextElementSibling;
        while (nextNode) {
          const nextTextSource = DocumentUtil.findContainingTextSource(nextNode);
          if (nextTextSource) {
            return nextTextSource;
          }
        ...
        // If no valid next text source is found, traverse up the DOM tree
        const parentNode = currentNode.parentNode;
        if (parentNode && parentNode !== document.body) {
          return this._getNextTextSource(parentNode);
        }
      
        return null;
      };
      _getAboveTextSource = function (currentNode) {
        let parentNode = currentNode.parentElement;
        while (parentNode && parentNode !== document.body) {
          const siblingNodes = Array.from(parentNode.childNodes);
          const currentIndex = siblingNodes.indexOf(currentNode);
      
          for (let i = currentIndex - 1; i >= 0; i--) {
            const aboveTextSource = DocumentUtil.findContainingTextSource(siblingNodes[i]);
            if (aboveTextSource) {
              return aboveTextSource;
            }
      
            // Check if the sibling node has child nodes
            if (siblingNodes[i].childNodes.length > 0) {
              const lastChild = siblingNodes[i].childNodes[siblingNodes[i].childNodes.length - 1];
              const lastChildTextSource = DocumentUtil.findContainingTextSource(lastChild);
              if (lastChildTextSource) {
                return lastChildTextSource;
              }
            }
          }
      ...
      _getBelowTextSource = function (currentNode) {
        let parentNode = currentNode.parentElement;
        while (parentNode && parentNode !== document.body) {
          const siblingNodes = Array.from(parentNode.childNodes);
          const currentIndex = siblingNodes.indexOf(currentNode);
      
          for (let i = currentIndex + 1; i < siblingNodes.length; i++) {
            const belowTextSource = DocumentUtil.findContainingTextSource(siblingNodes[i]);
            if (belowTextSource) {
              return belowTextSource;
            }
      
            // Check if the sibling node has child nodes
            if (siblingNodes[i].childNodes.length > 0) {
              const firstChild = siblingNodes[i].childNodes[0];
              const firstChildTextSource = DocumentUtil.findContainingTextSource(firstChild);
              if (firstChildTextSource) {
                return firstChildTextSource;
              }
            }
          }
